...
describe 'GitHub Action Integration Tests' do
  ...
  describe 'Docker integration' do
    before(:all) do
      ...
      # Create test images
      `magick convert -size 1200x800 xc:red #{@test_images_dir}/large.jpg`
      `magick convert -size 500x400 xc:blue #{@test_images_dir}/small.jpg`
      `magick convert -size 1024x1024 xc:green #{@test_images_dir}/square.png`
    end
  ...
  describe 'GitHub Actions environment simulation' do
    before do
      ...
      # Create test images
      `magick convert -size 1500x1000 xc:red #{@test_images_dir}/oversized.jpg`
      `magick convert -size 800x600 xc:blue #{@test_images_dir}/normal.jpg`
    end
    ...
    it 'handles no changes scenario' do
      ...
      `magick convert -size 500x400 xc:green #{@test_images_dir}/small1.jpg`
      `magick convert -size 600x500 xc:yellow #{@test_images_dir}/small2.jpg`
  ...
  describe 'action performance and reliability' do
    it 'handles large number of images' do
      ...
      10.times do |i|
        `magick convert -size 1200x800 xc:red #{test_images_dir}/test#{i}.jpg`
      end
    ...
    it 'handles different image formats correctly' do
      ...
      `magick convert -size 1200x800 xc:red #{test_images_dir}/test.jpg`
      `magick convert -size 1200x800 xc:blue #{test_images_dir}/test.jpeg`
      `magick convert -size 1200x800 xc:green #{test_images_dir}/test.png`
